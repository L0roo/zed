...
def filter_points(point_cloud,cam_num):
    ...
    if rotate:
        ...
        if cam_num == 1:
            ...
            hom_transformed = (hrot_matrix_2 @ homogeneous_points.T).T
        else:
            print("Error: Only two rot matrices")
...
def main():
    ...
    init_parameters.set_from_svo_file(input_path_1)
    init_parameters.depth_mode = sl.DEPTH_MODE.NEURAL_PLUS
    # Open the ZED
    zed = sl.Camera()
    err = zed.open(init_parameters)

    init_parameters2 = sl.InitParameters()
    init_parameters2.set_from_svo_file(input_path_2)
    init_parameters2.depth_mode = sl.DEPTH_MODE.NEURAL_PLUS
    # Open the ZED
    zed2 = sl.Camera()
    err2 = zed2.open(init_parameters2)
        ...
        if zed.grab() == sl.ERROR_CODE.SUCCESS:
            if zed2.grab() == sl.ERROR_CODE.SUCCESS:
                ...
                if i % downsampling == 0:
                    ...
                    zed2.retrieve_measure(point_cloud2, sl.MEASURE.XYZRGBA)
                    st_filter = time.time()
                    points1 = filter_points(point_cloud,1)
                    points2 = filter_points(point_cloud2,2)
                    points2[:,:3]+=sec_shift_vec *scale
                ...
                elif zed2.grab() == sl.ERROR_CODE.END_OF_SVOFILE_REACHED:
